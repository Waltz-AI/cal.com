...
export default class EventManager {
  ...
  constructor(
    ...
    this.calVideo = user.calVideo ?? null;
  ...
  public async create(event: CalendarEvent): Promise<CreateUpdateResult> {
    ...
    if (!evt.location) {
      ...
      const calVideoKeys = calVideoKeysSchema.safeParse(this.calVideo?.keys);

      if (this.calVideo?.enabled && calVideoKeys.success) evt["location"] = "integrations:daily";
    }
  ...
  public async reschedule(
    ...
    if (bookingPayment && newBookingId) {
      ...
      await this.prismaReadService.prisma.payment.updateMany({
        where: {
          id: {
            in: paymentIds,
          },
  ...
  private async updateAllCalendarEvents(
    ...
    try {
      ...
      let newBooking;
      if (newBookingId) {
        newBooking = await this.prismaReadService.prisma.booking.findUnique({
          where: {
            id: newBookingId,
          },
          select: {
  ...
  private async createAllCRMEvents(event: CalendarEvent) {
    ...
    for (const credential of this.crmCredentials) {
      ...
      let success = true;
      const createdEvent = await crm.createEvent(event).catch((error: any) => {
        success = false;
  ...
  private async updateAllCRMEvents(event: CalendarEvent, booking: PartialBooking) {
    ...
    for (const reference of booking.references) {
      ...
      if (credential) {
        ...
        const updatedEvent = await crm.updateEvent(reference.uid, event).catch(() => {
          success = false;
        });
